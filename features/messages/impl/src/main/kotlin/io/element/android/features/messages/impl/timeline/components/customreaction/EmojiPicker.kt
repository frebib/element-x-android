    ...
    Row(
        ...
        MessagesReactionButton(
            content = MessagesReactionsButtonContent.Text(
                text = searchQuery.ellipsize(MAX_REACTION_LENGTH_CHARS),
                highlight = true,
            ),
            onClick = { onReactionSelected(searchQuery) },
            onLongClick = {},
        )
    }
    HorizontalDivider(
...
internal fun EmojiPickerPreview() = ElementPreview {
    EmojiPicker(
        ...
        state = EmojiPickerState(isSearchActive = false, searchQuery = "", searchResults = SearchBarResultState.Initial()) {},
        modifier = Modifier.fillMaxWidth(),
    )
}
